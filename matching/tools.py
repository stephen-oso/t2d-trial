...
def search_trials(query: str) -> str:
    ...
    try:
        ...
        logger.warning("search_trials failed: %s", exc)
        return json.dumps([])
...
def check_eligibility(input_json: str) -> str:
    ...
    try:
        ...
        logger.warning("check_eligibility: bad input JSON: %s", exc)
        return json.dumps([{"criterion": "invalid input", "status": "UNKNOWN", "patient_value": None}])
    ...
    try:
        url = f"{_CTGOV_BASE}/studies/{trial_id}"
        req = urllib.request.Request(url, headers={"Accept": "application/json"})
        with urllib.request.urlopen(req, timeout=30) as resp:
            study_data = json.loads(resp.read())
    except Exception as exc:
        logger.warning("check_eligibility: CT.gov fetch failed for %s: %s", trial_id, exc)
        return json.dumps([{"criterion": "eligibility criteria unavailable", "status": "UNKNOWN", "patient_value": None}])
...
Return ONLY the JSON array, no explanation or other text."""
    ...
    try:
        ...
        logger.warning("check_eligibility: Anthropic call failed for %s: %s", trial_id, exc)
        return json.dumps([{"criterion": "evaluation failed", "status": "UNKNOWN", "patient_value": None}])
